...
function getRectangleString(width, height) {
    let horizontal = (width-2 !== 0) ? '─'.repeat(width-2) : '';
    let vertical = (height-2 !== 0) ? ' '.repeat(width-2) : '';
...
function encodeToRot13(str) {
    ...
    for (let i = 0; i < str.length; i++){
        ...
        if (str.charCodeAt(i) >= 65 && str.charCodeAt(i) <= 90 && newCharCode > 90 ){
            newCharCode -= 26;
        }
        if (str.charCodeAt(i) >= 97 && str.charCodeAt(i) <= 122 && newCharCode > 122 ){
            newCharCode -= 26;
        }
